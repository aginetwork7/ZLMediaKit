...
// Parse temporary recording filename generated by MP4Recorder createFile():
// ".<YYYY-MM-DD-HH-MM-SS>-<index>.mp4"
// Returns false if the name doesn't match the temp naming scheme.
inline bool parseTempRecordFileName(const std::string &name, time_t &start_sec, std::string *index_str = nullptr) {
    if (name.size() <= 1 + kRecordTimeStrLen + kRecordFileSuffixLen || name[0] != '.' ||
        name.compare(name.size() - kRecordFileSuffixLen, kRecordFileSuffixLen, kRecordFileSuffix) != 0) {
        return false;
    }

    auto base = name.substr(1, name.size() - 1 - kRecordFileSuffixLen);
    ...
    if (index_str) {
        ...
        *index_str = "0";
        if (last_dash != std::string::npos && last_dash >= kRecordTimeStrLen) {
            *index_str = base.substr(last_dash + 1);
        }
    }
    return true;
}
